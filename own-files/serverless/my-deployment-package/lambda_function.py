...
def lambda_handler(event, context):
    ...
    s3_client.upload_file(
        "/tmp/pixelated-8x8-{}".format(object_key),
        processed_bucket,
        "pixelated-8x8-{}".format(key),
    )
    s3_client.upload_file(
        "/tmp/pixelated-16x16-{}".format(object_key),
        processed_bucket,
        "pixelated-16x16-{}".format(key),
    )
    s3_client.upload_file(
        "/tmp/pixelated-32x32-{}".format(object_key),
        processed_bucket,
        "pixelated-32x32-{}".format(key),
    )
    s3_client.upload_file(
        "/tmp/pixelated-48x48-{}".format(object_key),
        processed_bucket,
        "pixelated-48x48-{}".format(key),
    )
    s3_client.upload_file(
        "/tmp/pixelated-64x64-{}".format(object_key),
        processed_bucket,
        "pixelated-64x64-{}".format(key),
    )
